...
// Phương thức để thực hiện thao tác kéo thả theo % màn hình
	public void swipeByScreenPercentage(int direction, int screenPercentage, Duration duration) {
		...
		switch (direction) {
			case Constants.SWIPE_UP:
				startx = size.width/2;
				starty = size.height * screenPercentage / 100;
				endx = startx;
				break;
			case Constants.SWIPE_RIGHT:
				starty = size.height/2;
				endx = size.width * screenPercentage / 100;
				endy = starty;
				break;
			case Constants.SWIPE_DOWN:
				startx = size.width/2;
				endy = size.height * screenPercentage / 100;
				endx = startx;
				break;
			case Constants.SWIPE_LEFT:
				starty = size.height/2;
				startx = size.width * screenPercentage / 100;
				endy = starty;
				break;
			default: 
				break;
